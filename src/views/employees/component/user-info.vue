      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <ImageUpload ref="userInfoImgRef"></ImageUpload>
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <ImageUpload ref="formDataImgRef"></ImageUpload>
        </el-form-item>
...
export default {
  ...
  async mounted() {
    ...
    if (this.userInfo.staffPhoto) {
      this.$refs.userInfoImgRef.fileList = [{ url: this.userInfo.staffPhoto }]
    }
    this.formData = await getPersonalDetail(this.userId)
    if (this.formData.staffPhoto) {
      this.$refs.formDataImgRef.fileList = [{ url: this.formData.staffPhoto }]
    }
  },
  async created() {
    // 回显数据
  ...
  methods: {
    async saveUser() {
      const { fileList, isUploading } = this.$refs.userInfoImgRef
    ...
    async savePersonal() {
      const { fileList, isUploading } = this.$refs.formDataImgRef
      if (fileList.length > 0) {
